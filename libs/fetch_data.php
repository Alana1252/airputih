...
function getbottomslidermember($table){
	require("database/db_connect.php");
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $sliderposts => $slideritem) {
      	echo '<li>
			...
			<h3>'.$slideritem['username'].'
			<span><i>'.$slideritem['discord'].'</i>
			<label>|</label>
			<i>'.$slideritem['jabatan'].'</i>
			</span>
			</h3>
...
function getfour($table){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $thefour => $fourdata) {
      	# code...
			echo '<li>
			<p href="blogadmin/images/'.$fourdata['photo'].'">
...
function getonelatest($table){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table WHERE status='enable'";
	if ($result=mysqli_query($con,$sql))
	{
      	//count number of rows in query result
		$rowcount=mysqli_num_rows($result);
  ...
  <div class="row">
    <div class="col">
	<img src="blogadmin/images/'.$onedata['photo'].'" class="img-fluid ml-2" alt="event" style="width:400px;height:200px">
	...
	<p class="h4"'.$onedata['id_event'].'">'.$onedata['title'].'</p>
	<p class="bi bi-alexa">Event status : '.$onedata['event_status'].'</p>
	<p class="">'.$onedata['content'].'</p>
    </div>
    <div class="col">
    <p class="h4"'.$onedata['id_event'].'">'.$onedata['pemenang'].'</p>
    </div>
  </div>
  <div class="row">
    <div class="col-5 ml-2">
      <a><i class="far fa-user"></i> Created by : '.$onedata['author'].'</a><p></p>
	  <a><i class="bi bi-cash-coin"></i> Sponsored by : '.$onedata['sponsor'].'</a>
    </div>
    <div class="col-3">
